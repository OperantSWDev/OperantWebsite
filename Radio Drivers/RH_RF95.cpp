...
//
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
// These are modified by RKing for FleetLink on 3/18/16
// This .cpp file was modified to create a command sendWithDelay()
// The point of this command is to send a radio transmission after a specified delay only if there is no Channel Activity
// This required the addion of the CAD ("Channel Activity Detection") functionality of the SX1272 IC
// This is a native functionality that isn't supported in the RadioHead library
// This required trhe following changes:
//     - sendWithDelay() function added as the top level functionality
//     - setModeCAD() added to initiate a CAD Activity
//     - Existing interruptHandler() function extended to set a global CAD Detected flag after a CAD Done interrupt
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//
//
// Copyright (C) 2011 Mike McCauley
// $Id: RH_RF95.cpp,v 1.6 2015/01/02 21:38:24 mikem Exp $
...
// RK changes
// this global flag added here rather than RHGenericDriver.cpp to avoid modifying more files; kind of awkward though
bool _cadDetectFlag = false;
// End RK changes
...
PROGMEM static const RH_RF95::ModemConfig MODEM_CONFIG_TABLE[] =
{
    ...
    { 0x88,   0xc4,    0x04}, // Bw125Cr48Sf4096  RKing likes Bw500Cr48sf496 and thinks AGC bit dhould be ON in reg 26, was     { 0x78,   0xc4,    0x0}
...
bool RH_RF95::init()
{
  ...
  //  Serial.print("int pin:");Serial.println(_interruptPin);
  //  Serial.print("int number:");Serial.println(interruptNumber);
...
void RH_RF95::setModeCAD()
{
    ...
    {
      setModeIdle();
      spiWrite(RH_RF95_REG_01_OP_MODE, RH_RF95_MODE_CAD); // CAD Init
      spiWrite(RH_RF95_REG_40_DIO_MAPPING1, 0x80); // Interrupt on CAD Done (determined by experiment)
	    _mode = RHModeCAD;
    }
}
// end RK change
